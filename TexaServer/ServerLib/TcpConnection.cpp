...
void TcpConnection::RealSend(const BufferPtr& buffer)
{
    ...
    writting_msg_ = true;
    socket_.Send(std::bind(&TcpConnection::OnWrite, shared_from_this(), std::placeholders::_1, std::placeholders::_2),
...
void TcpConnection::OnWrite(const ErrorCode& e, uint32 len)
{
    ...
    if (!send_buf_.empty()) {
        ...
        writting_msg_ = true;
        socket_.Send(
            std::bind(&TcpConnection::OnWrite, shared_from_this(), std::placeholders::_1, std::placeholders::_2),
            &(*buffer->begin()), buffer->size());
    }
}
