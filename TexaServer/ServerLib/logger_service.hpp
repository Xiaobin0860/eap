...
public:
    ...
    struct logger_impl {
        explicit logger_impl(const std::string& ident)
            : identifier(ident)
        {
        }
    ...
    logger_service(boost::asio::io_service& io_service)
        ...
        , work_thread_(new std::thread(boost::bind(&boost::asio::io_service::run, &work_io_service_)))
    {
    }
    ...
    void use_file(impl_type& /*impl*/, const std::string& file)
    {
        ...
        work_io_service_.post(boost::bind(&logger_service::use_file_impl, this, file));
    }
    ...
    void log(impl_type& /*impl*/, const std::string& message)
    {
        ...
        time_t t = time(0);
        tm*    lt = localtime(&t);
        sprintf(current_time_buffer, "%02d:%02d:%02d", lt->tm_hour, lt->tm_min, lt->tm_sec);
        os << current_time_buffer << " " << message;

        // Pass the work of opening the file to the background thread.
        work_io_service_.post(boost::bind(&logger_service::log_impl, this, os.str()));
    }
